...
# %% Loading the song
#Path to the song
path_temp =  'Documents/Cours/MSDS/S1/Statistique Bayesienne/MusicAnalysis-Bayesian_DynamicModel.nosync/Data/beatles.wav' #Mettre le mP3 ici
#Loading using librosa
amplitude, sr = librosa.load(path_temp)


#%%Plot of the amplitude using LIBROSA
plt.figure(figsize=(10, 4))
librosa.display.waveplot(amplitude, sr, alpha=0.8)
plt.title('Waveform : amplitude ')

# %% Melspectrograme LIBROSA
...
#%% Compute MFCCs
mfccs = librosa.feature.mfcc(y=amplitude, sr=sr, n_mfcc=10, norm="ortho")
plt.figure(figsize=(10, 5))
plt.imshow(mfccs,cmap=plt.cm.jet, aspect='auto')

librosa.display.specshow(mfccs, sr=sr, hop_length=500, x_axis='time')
#%% Time conversion
